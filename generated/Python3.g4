...
comp_op: '<'|'>'|'=='|'>='|'<=' | '!=';
arith_expr: term (('+'|'-') term)*;                     //出现加减法
term: factor (('*'|'/'|'//'|'%') factor)*;              //乘除法优先级比加减高
factor: ('+'|'-') factor | atom_expr;                   //此处+-代表正负，优先级最高
...
/// bytesliteral(字符串文字)   ::=  bytesprefix(shortbytes | longbytes)
/// bytesprefix(字符串前缀)    ::=  "b" | "B" | "br" | "Br" | "bR" | "BR" | "rb" | "rB" | "Rb" | "RB"
BYTES_LITERAL
